...
clean requests (``photolibraryclean``) — a destructive "wipe this library's
content" action, gated behind an emailed, single-use token per required
approver (the owner, plus one other member picked at random if the library is
shared):
    request:      PK=``library_id``       RK=``request_id``  -> {requestedBy,
                   requiredUserIds, confirmedUserIds, status, createdAt, expiresAt}
    token lookup: PK=``token``            RK=``token_hash``  -> {requestId,
                   libraryId, userId}
    send throttle:PK=``throttle``         RK=``library_id``  -> {lastSentAt, window...}
"""
...
# A library-clean confirmation request must be acted on quickly: it's a
# destructive action, and a long-lived link sitting in an inbox is a bigger
# risk than the inconvenience of asking the owner to retry.
CLEAN_REQUEST_TTL_SECONDS = 30 * 60

# Clean-request-send throttle (mirrors the invite throttle), per library.
_CLEAN_MIN_INTERVAL_SECONDS = 60
_CLEAN_MAX_PER_HOUR = 5
...
def new_clean_request_id() -> str:
    return 'clean' + uuid.uuid4().hex
...
class LibraryStore:
    ...
    def __init__(
        ...
        *,
        ...
        self.clean_requests = clean_requests_table
    ...
    def set_cleanup_in_progress(self, library_id: str, job_id: str) -> None:
        """Mark a library as having a cleanup job in progress."""
        entity = self.get_library(library_id)
        if not entity:
            return
        entity['lastCleanupJobId'] = str(job_id)
        entity['lastCleanupStatus'] = 'in_progress'
        entity['lastCleanupStartTime'] = _now()
        entity['lastCleanupError'] = ''
        entity['PartitionKey'] = _LIBRARY_PK
        entity['RowKey'] = str(library_id)
        self.libraries.upsert_entity(entity)

    def set_cleanup_completed(self, library_id: str, photos_deleted: int = 0, blobs_deleted: int = 0) -> None:
        """Mark a library's cleanup as completed."""
        entity = self.get_library(library_id)
        if not entity:
            return
        entity['lastCleanupStatus'] = 'completed'
        entity['lastCleanupTime'] = _now()
        entity['lastCleanupPhotosDeleted'] = photos_deleted
        entity['lastCleanupBlobsDeleted'] = blobs_deleted
        entity['PartitionKey'] = _LIBRARY_PK
        entity['RowKey'] = str(library_id)
        self.libraries.upsert_entity(entity)

    def set_cleanup_failed(self, library_id: str, error: str = '') -> None:
        """Mark a library's cleanup as failed."""
        entity = self.get_library(library_id)
        if not entity:
            return
        entity['lastCleanupStatus'] = 'failed'
        entity['lastCleanupTime'] = _now()
        entity['lastCleanupError'] = str(error)
        entity['PartitionKey'] = _LIBRARY_PK
        entity['RowKey'] = str(library_id)
        self.libraries.upsert_entity(entity)
    ...
    # -- library clean (destructive, multi-party-confirmed content wipe) ------
    def get_active_clean_request(self, library_id: str) -> Optional[Dict]:
        """The library's pending, unexpired clean request, if any (one at a time)."""
        if self.clean_requests is None:
            return None
        now = _now()
        for row in self._query(self.clean_requests, f"PartitionKey eq '{library_id}'"):
            if str(row.get('status') or '') != 'pending':
                continue
            if int(row.get('expiresAt', 0) or 0) <= now:
                continue
    ...
    def create_clean_request(
        self,
        *,
        library_id: str,
        requested_by: str,
        required_user_ids: List[str],
        ttl_seconds: int = CLEAN_REQUEST_TTL_SECONDS,
    ) -> tuple:
        """Create a pending clean request needing every id in required_user_ids
        to confirm via their own emailed, single-use token. Returns
        ``(request_id, {user_id: raw_token})``."""
        request_id = new_clean_request_id()
        expires_at = _now() + int(ttl_seconds)
        tokens_by_user: Dict[str, str] = {}
        for user_id in required_user_ids:
            raw = generate_invite_token()
            token_hash = hash_invite_token(raw)
            tokens_by_user[str(user_id)] = raw
            self.clean_requests.upsert_entity({
                'PartitionKey': _TOKEN_PK,
                'RowKey': token_hash,
        ...
        self.clean_requests.upsert_entity({
            ...
            'requiredUserIds': ','.join(str(u) for u in required_user_ids),
            'confirmedUserIds': '',
            'status': 'pending',
            'createdAt': _now(),
            'expiresAt': expires_at,
    ...
    def is_clean_request_fully_confirmed(request: Dict) -> bool:
        required = {u for u in str(request.get('requiredUserIds') or '').split(',') if u}
        confirmed = {u for u in str(request.get('confirmedUserIds') or '').split(',') if u}
        return bool(required) and required <= confirmed

    def cancel_clean_request(self, library_id: str, request_id: Optional[str] = None) -> None:
        """Delete the library's active clean request (and any outstanding
        per-approver tokens for it). If ``request_id`` is omitted, cancels
        whatever is currently active."""
        if self.clean_requests is None:
            return
        request = self._get(self.clean_requests, library_id, request_id) if request_id else self.get_active_clean_request(library_id)
        if not request:
            return
        # Token rows are keyed by hash, which we no longer have; sweep the
        # library's token partition instead. Clean requests are rare and this
        # partition only ever holds a couple of rows at a time.
        for row in self._query(self.clean_requests, f"PartitionKey eq '{_TOKEN_PK}'"):
            if str(row.get('requestId') or '') == str(request.get('RowKey') or ''):
                self._delete(self.clean_requests, _TOKEN_PK, str(row.get('RowKey') or ''))
        self._delete(self.clean_requests, str(library_id), str(request.get('RowKey') or ''))

    def clean_request_send_allowed(self, library_id: str) -> bool:
        """Rate-limit clean-request emails per library. Records the send when
        allowed. Fails closed if the throttle row can't be written."""
        if self.clean_requests is None:
            return False
        now = _now()
        entity = self._get(self.clean_requests, _THROTTLE_PK, str(library_id)) or {
            'PartitionKey': _THROTTLE_PK,
            'RowKey': str(library_id),
        }
        last_sent = int(entity.get('lastSentAt', 0) or 0)
        window_start = int(entity.get('windowStart', 0) or 0)
        window_count = int(entity.get('windowCount', 0) or 0)
        if now - last_sent < _CLEAN_MIN_INTERVAL_SECONDS:
            return False
        if now - window_start >= 3600:
            window_start = now
            window_count = 0
        if window_count >= _CLEAN_MAX_PER_HOUR:
            return False
        entity['lastSentAt'] = now
        entity['windowStart'] = window_start
        entity['windowCount'] = window_count + 1
        try:
            self.clean_requests.upsert_entity(entity)
        except Exception:
            return False
        return True
