...
def _unstick_claimed_steps_still_running(
    user_id: str, filename: str, claimed_steps: Optional[List[str]], error: str,
) -> None:
    """Safety net for a crashed processing attempt.

    _apply_client_processing_results persists per-step status via ONE final
    upsert at the end of that function; if anything raises before reaching
    it, every step claim_processing_lease marked 'running' for this call is
    left wedged there forever -- release_processing_lease (called from the
    caller's own except/finally) only clears lease ownership fields, never
    resets step statuses. Nothing else ever revisits a 'running' step whose
    lease is already released, so without this it never retries (confirmed
    live 2026-09-14: 3 uploads stuck 'running' on every step for 2+ days,
    with real exif/ai_vision/geocode data already computed and written via
    mark_step_done's own immediate upserts, but the status fields themselves
    never resolved). Re-reads fresh so this doesn't clobber any such
    immediate writes the crashed attempt already made for other steps.
    """
    metadata_table_client = _CTX.get('metadata_table_client')
    if metadata_table_client is None:
        return
    ...
    if str(entity.get('processing_state') or '').strip().lower() == 'deleted':
        return
    steps = claimed_steps if claimed_steps is not None else list(BROWSER_PROCESSING_STEPS)
    changed = False
    for step in steps:
        if step not in BROWSER_PROCESSING_STEPS:
            continue
        field = f'{step}_status'
        if str(entity.get(field) or '').strip().lower() == 'running':
            entity[field] = 'failed'
            changed = True
    if not changed:
        return
    entity['last_error'] = f'processing_failed_before_status_commit: {error}'[:1000]
