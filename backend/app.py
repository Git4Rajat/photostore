...
@app.route('/photos/lookup-batch', methods=['POST'])
@app.route('/api/photos/lookup-batch', methods=['POST'])
def lookup_photos_batch():
    ...
    for raw_name in raw:
        ...
        if not metadata:
            continue
        photos.append(_build_photo_summary(user_id, safe_name, metadata, include_props=False, pid_to_name=pid_to_name))
...
@app.route('/api/tools/workbench/actions', methods=['POST'])
def record_workbench_action():
    ...
    if not action or not isinstance(steps, list) or not steps or scope not in ('selected', 'view', 'library'):
        return jsonify({'error': 'action, steps, and a valid scope are required', 'code': 'invalid_action'}), 400
    try:
    ...
    try:
        workbench_actions_table_client.upsert_entity({
            'PartitionKey': user_id,
            'RowKey': action_id,
            'action': action,
        ...
        pass
    return jsonify({'success': True, 'actionId': action_id})


@app.route('/api/tools/workbench/actions', methods=['GET'])
def list_workbench_actions():
    ...
    try:
        rows_iter = workbench_actions_table_client.query_entities(
            f"PartitionKey eq '{_escape_odata(user_id)}'",
            select=select_cols,
        )
    except Exception:
    ...
    return jsonify({'actions': rows[:100]})


@app.route('/api/tools/workbench/actions/<action_id>', methods=['GET'])
def get_workbench_action(action_id: str):
    ...
    if error:
        return error
    try:
        row = workbench_actions_table_client.get_entity(partition_key=user_id, row_key=action_id)
    except Exception:
        return jsonify({'error': 'Not found'}), 404
    return jsonify({'filenames': json.loads(row.get('filenames', '[]') or '[]')})
