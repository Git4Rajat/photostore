...
def test_clean_request_solo_owner_needs_only_their_own_confirmation():
    s = make_store()
    owner = _seed_owner(s)
    request_id, tokens = s.create_clean_request(
        library_id=owner, requested_by=owner, required_user_ids=[owner],
    )
    assert set(tokens.keys()) == {owner}
...
def test_clean_request_send_throttle_caps_rate():
    s = make_store()
    owner = _seed_owner(s)
    assert s.clean_request_send_allowed(owner) is True
    # Immediately retrying is blocked by the minimum-interval throttle.
    assert s.clean_request_send_allowed(owner) is False
