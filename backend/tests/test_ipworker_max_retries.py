...
def test_dispatch_exception_returns_error_not_done(metadata_table, monkeypatch):
    """A genuine processing failure (DB write error, transient dependency
    failure, etc.) must not be treated the same as a completed job -- run_ipworker
    only redelivers on outcome != 'done'/'lease_busy'-exhausted, so forcing
    'done' here (the pre-fix behavior) deleted the message on the very first
    failed attempt with zero retries, silently dropping the photo forever.
    'error' lets it redeliver, bounded by the dequeue_count>IPWORKER_MAX_RETRIES
    check above on the next attempt.
    """
    def _raise(payload, job_id, user_id):
        raise RuntimeError('boom')

    monkeypatch.setattr(app, '_handle_ipwork_queue_payload', _raise)
    message = _FakeQueueMessage(
        {'jobId': 'ipwork:u1:f1', 'user_id': 'u1', 'filename': 'f1.jpg'},
        dequeue_count=1,
    )
...
def test_dispatch_exception_past_max_retries_still_drops(metadata_table, monkeypatch):
    """A message that keeps failing forever must still terminate once
    dequeue_count exceeds IPWORKER_MAX_RETRIES -- the retry window from the
    fix above is bounded, not infinite."""
    def _raise(payload, job_id, user_id):
        raise RuntimeError('boom')
