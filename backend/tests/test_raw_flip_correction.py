...
def _marker_jpeg_with_exif_orientation(width: int, height: int, orientation: int) -> bytes:
    image = Image.new('RGB', (width, height), (0, 0, 0))
    marker_size = max(1, min(width, height) // 4)
    for x in range(marker_size):
        for y in range(marker_size):
...
def test_normalize_preview_bytes_does_not_double_rotate_when_source_has_exif_orientation():
    # Regression for IMG_9206.DNG (2026-09-14): every _normalize_preview_bytes
    # call site immediately applies _apply_raw_flip(raw_flip) afterward, which
    # is meant to be the single authoritative rotation. Most extracted RAW
    # previews carry no orientation tag of their own (Orientation=1), but at
    # least one real Apple ProRAW DNG's extracted PreviewImage carried a
    # genuine, correct Orientation tag -- if _normalize_preview_bytes also
    # auto-rotated via that tag, the subsequent _apply_raw_flip rotated the
    # already-correct image a second time, landing on a wrong orientation.
    # _normalize_preview_bytes must pass the pixels through unrotated so
    # _apply_raw_flip's rotation is the only one that happens.
    source = _marker_jpeg_with_exif_orientation(80, 40, orientation=6)
    normalized = iu._normalize_preview_bytes(source)
    assert _corner_with_marker(normalized) == _corner_with_marker(source) == 'top-left'
