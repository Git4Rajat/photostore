...
def test_extract_raw_preview_bytes_prefers_structural_parser_over_blind_scan(monkeypatch):
    # Apple ProRAW DNGs store compressed RAW sensor-tile data immediately
    # before the real embedded preview; a stray marker-like byte in that tile
    # data can make the blind byte-scan (extract_embedded_jpeg) return a
    # decodable-but-wrong monochrome auxiliary image instead of the real color
    # preview (confirmed against a real file, 2026-09-14). The structural path
    # (_temporary_preview_from_bytes -> exiftool/rawpy via a temp file, same as
    # the already-safe extract_raw_preview_from_path) must be tried first, so
    # its result wins even when the blind scan would also return something
    # "good enough".
    good = _solid_jpeg(3000, 2000)
    wrong = _solid_jpeg(1500, 2000)
    monkeypatch.setattr(iu, '_temporary_preview_from_bytes', lambda image_bytes, filename: good)
    monkeypatch.setattr(iu, 'extract_embedded_jpeg', lambda image_bytes, flip: wrong)
    monkeypatch.setattr(iu, '_extract_rawpy_preview_from_bytes', lambda image_bytes: None)

    result = iu.extract_raw_preview_bytes(b'fake raw bytes', 'IMG_9206.DNG')

    assert result == good


def test_extract_raw_preview_bytes_falls_back_to_blind_scan_without_filename(monkeypatch):
    # No filename means _temporary_preview_from_bytes can't pick a temp-file
    # suffix exiftool would recognize, so the blind scan is the only option.
    wrong = _solid_jpeg(1500, 2000)
    monkeypatch.setattr(iu, 'extract_embedded_jpeg', lambda image_bytes, flip: wrong)
    monkeypatch.setattr(iu, '_extract_rawpy_preview_from_bytes', lambda image_bytes: None)

    result = iu.extract_raw_preview_bytes(b'fake raw bytes')
