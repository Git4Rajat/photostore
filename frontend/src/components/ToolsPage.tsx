...
type ProcessingFilterProcess = 'all' | 'preview' | 'thumbnail' | 'exif' | 'ocr' | 'aiVision' | 'mapDetection' | 'face';
type ChipStepKey = 'preview' | 'thumbnail' | 'exif' | 'ocr' | 'aiVision' | 'mapDetection' | 'face';
...
type QueueStatus = {
    preview?: { queued?: number; pending?: number; pendingTotal?: number; running?: number; failed?: number; noData?: number };
...
const queueStageCards: QueueStageConfig[] = [
    { key: 'preview', label: 'Preview', description: 'Shrunk lightbox preview', icon: processingStageIcons.preview },
...
const browserActionButtons: Array<{ action: BrowserProcessingAction; label: string; icon: React.ComponentType<React.SVGProps<SVGSVGElement>> }> = [
    { action: 'preview', label: 'Preview', icon: processingStageIcons.preview },
...
const ToolsPage: React.FC = () => {
    ...
    const [selectedStepActions, setSelectedStepActions] = useState<Set<BrowserProcessingAction>>(new Set());
    ...
    const [showProcessingFilterMenu, setShowProcessingFilterMenu] = useState<boolean>(false);
    const processingFilterMenuRef = useRef<HTMLDivElement | null>(null);
    const [gallerySearchInput, setGallerySearchInput] = useState<string>('');
    const [gallerySearchQuery, setGallerySearchQuery] = useState<string>('');
    const [gallerySearchOpen, setGallerySearchOpen] = useState<boolean>(false);
    const gallerySearchRef = useRef<HTMLDivElement | null>(null);
    ...
    const [pendingFocusFilenames, setPendingFocusFilenames] = useState<string[]>([]);
    ...
    const loadMorePhotos = async () => {
        ...
        setLoadingMore(true);
        try {
            const trimmedQuery = gallerySearchQuery.trim();
            const response = trimmedQuery
                ? await get(`/photos/search?q=${encodeURIComponent(trimmedQuery)}&offset=${photosOffset}&limit=${PAGE_SIZE}`)
                : await get(`/photos?offset=${photosOffset}&limit=${PAGE_SIZE}`);
            const fetched = Array.isArray(response?.photos) ? response.photos : [];
    ...
    const submitGallerySearch = (override?: string) => {
        const nextQuery = (override ?? gallerySearchInput).trim();
        setGallerySearchQuery(nextQuery);
        void loadPhotos(nextQuery);
    };

    const closeGallerySearch = () => {
        if (!gallerySearchInput.trim() && gallerySearchQuery) {
    ...
    useEffect(() => {
        if (!showProcessingFilterMenu) return;
        const handlePointerDown = (event: PointerEvent) => {
            if (processingFilterMenuRef.current && !processingFilterMenuRef.current.contains(event.target as Node)) {
                setShowProcessingFilterMenu(false);
            }
        };
        const handleKeyDown = (event: KeyboardEvent) => {
            if (event.key === 'Escape') setShowProcessingFilterMenu(false);
        };
    ...
    // "Open in Workbench" deep link from another page: pull those photos in
    // (they may not be on the first loaded page), select them, and switch to
    // the 'all' view so they aren't hidden by the 'recent'/'attention' filters.
    // Accepts the current plural `filenames` (comma-joined) param, falling
    // back to the legacy singular `filename` param for old links.
    useEffect(() => {
        if (!isBrowserWorkbenchPage) {
            return;
        }
        const params = new URLSearchParams(location.search);
        const rawList = params.get('filenames');
        const rawSingle = params.get('filename');
        const targets = rawList
            ? rawList.split(',').map((s) => decodeURIComponent(s)).filter(Boolean)
            : (rawSingle ? [rawSingle] : []);
        if (targets.length === 0) {
            return;
        }
        setPendingFocusFilenames(targets);
        setViewMode('all');
        setSelected((prev) => {
    ...
    // Fetches and re-injects any deep-linked photos still missing from
    // `photos`, in one batched request. This also guards against the
    // mount-time loadPhotos() page-0 fetch (above) resolving *after* this one
    // and overwriting `photos` wholesale, which would otherwise silently drop
    // target photos that aren't on the first page.
    useEffect(() => {
        const missing = pendingFocusFilenames.filter((f) => !photos.some((p) => p.filename === f));
        if (missing.length === 0) {
            return undefined;
        }
        let cancelled = false;
        void (async () => {
            try {
                const response = await post('/api/photos/lookup-batch', { filenames: missing });
                const found: Photo[] = Array.isArray(response?.photos) ? response.photos : [];
                if (!cancelled && found.length > 0) {
                    setPhotos((prev) => {
                        const have = new Set(prev.map((p) => p.filename));
                        return [...found.filter((p) => !have.has(p.filename)), ...prev];
                    });
                }
            } catch {
                // Best effort — if the lookup fails those photos simply won't be pre-loaded.
            }
        })();
        return () => {
            cancelled = true;
        };
    }, [pendingFocusFilenames, photos]);
    ...
    // Only the first deep-linked photo gets scrolled to -- enough to orient
    // the user, since the rest are already visibly pre-checked via `selected`.
    useEffect(() => {
        const firstTarget = pendingFocusFilenames[0];
        if (!firstTarget || !workbenchPhotos.some((photo) => photo.filename === firstTarget)) {
            return undefined;
        }
    ...
    }, [pendingFocusFilenames, workbenchPhotos]);
    ...
    // Human-readable label for one or more selected steps, e.g. "Preview",
    // "Preview and Thumbnails", or "Preview, Thumbnails, and OCR".
    const describeStepActions = (actions: BrowserProcessingAction[]): string => {
        const labels = actions.map((action) => browserActionButtons.find((item) => item.action === action)?.label || action);
        if (labels.length <= 1) return labels[0] || 'processing';
        if (labels.length === 2) return `${labels[0]} and ${labels[1]}`;
        return `${labels.slice(0, -1).join(', ')}, and ${labels[labels.length - 1]}`;
    };

    const combinedStepsForActions = (actions: BrowserProcessingAction[]): string[] => {
        const steps = new Set<string>();
        actions.forEach((action) => browserProcessingActionSteps[action]?.forEach((step) => steps.add(step)));
    ...
    // processing pull. Mirrors runBackfillPhotos() but scoped via 'steps'.
    const runBrowserActionsOnLibrary = async (actions: BrowserProcessingAction[]) => {
        ...
        try {
            ...
            void postTools('/api/tools/workbench/actions', {
                ...
                steps,
                scope: 'library',
                filenameCount: queued,
            }).catch(() => {});
            void refreshWorkbenchHistory();
        } catch (err) {
            setMessage(`Library-wide '${label}' failed: ${String(err)}`);
        } finally {
            setRunning(null);
            setSelectedStepActions(new Set());
        }
    };

    // Runs every currently-selected step (see the "Processing steps" checkboxes)
    // as a single queued batch, instead of forcing one step at a time.
    const runSelectedBrowserActions = async () => {
        const actions = Array.from(selectedStepActions);
        if (actions.length === 0) {
            setMessage('Select at least one step to run.');
            return;
        }
        if (isBrowserWorkbenchPage && scope === 'library') {
            await runBrowserActionsOnLibrary(actions);
            return;
        }
        ...
        try {
            ...
            // selection, mirroring what runBrowserActionsOnLibrary already does for
            ...
            if (processingMode !== 'browser') {
                try {
                    ...
                    setMessage(`Backend enqueue for '${label}' failed: ${String(err)}`);
                }
            }
            ...
            setMessage(`Browser '${label}' failed: ${String(err)}`);
        } finally {
            setRunning(null);
            setSelectedStepActions(new Set());
        }
    ...
    const toggleStepAction = (action: BrowserProcessingAction) => {
        setSelectedStepActions((prev) => {
            const next = new Set(prev);
            if (next.has(action)) {
                next.delete(action);
            } else {
        ...
        return (
            <div className="tools-action-grid" aria-label="Processing steps">
                {browserActionButtons.map((item) => {
                    const isSelected = selectedStepActions.has(item.action);
                    return (
                        <button
                            key={item.action}
                            type="button"
                            className={`btn btn-soft tools-action-button${isSelected ? ' active' : ''}`}
                            onClick={() => toggleStepAction(item.action)}
                            disabled={isRunning}
                ...
                <button
                    type="button"
                    className="btn btn-primary tools-action-button tools-action-run-button"
                    onClick={() => void runSelectedBrowserActions()}
                    disabled={isRunning || selectedStepActions.size === 0}
                    aria-busy={isRunning}
                >
                    {isRunning ? 'Running…' : `Run selected${selectedStepActions.size > 0 ? ` (${selectedStepActions.size})` : ''}`}
                </button>
            </div>
        );
    };
    ...
    const processingFiltersActive = processingFilterState !== 'all' || processingFilterProcess !== 'all';

    const renderProcessingFilters = () => (
        <div className="gallery-menu-anchor" ref={processingFilterMenuRef}>
            <button
                type="button"
                onClick={() => setShowProcessingFilterMenu((prev) => !prev)}
                className={`btn icon-btn ${showProcessingFilterMenu || processingFiltersActive ? 'btn-primary' : 'btn-soft'}`}
                aria-label="Processing filters"
                aria-expanded={showProcessingFilterMenu}
                title="Filters"
            >
            ...
            {showProcessingFilterMenu && (
                <div className="gallery-menu gallery-menu-filters" role="menu" aria-label="Processing filters">
                    <p className="gallery-menu-label">Status</p>
                    <div className="gallery-menu-row">
                        {(Object.keys(processingStateLabels) as ProcessingFilterState[]).map((state) => (
                            <button
                                key={state}
                                type="button"
                                onClick={() => setProcessingFilterState(state)}
                                className={`btn btn-soft gallery-menu-btn ${processingFilterState === state ? 'active' : ''}`}
                            >
                                {processingStateLabels[state]}
                            </button>
                        ))}
                    </div>
                    <p className="gallery-menu-label">Process</p>
                    <div className="gallery-menu-row">
                        {(Object.keys(processingProcessLabels) as ProcessingFilterProcess[]).map((process) => (
                            <button
                                key={process}
                                type="button"
                                onClick={() => setProcessingFilterProcess(process)}
                                className={`btn btn-soft gallery-menu-btn ${processingFilterProcess === process ? 'active' : ''}`}
                            >
    ...
    const renderGallerySearchControl = () => (
        gallerySearchOpen ? (
            ...
            <button
                ...
                className={`btn icon-btn ${gallerySearchQuery ? 'btn-primary' : 'btn-soft'}`}
                aria-label="Search"
                title={gallerySearchQuery ? `Searching: ${gallerySearchQuery}` : 'Search'}
            >
    ...
    const renderPhotoStatusRow = (photo: Photo, index: number, withOpenButton: boolean) => (
        <div className="tools-photo-status-row" aria-label={`Processing status for ${photo.filename}`}>
            <PhotoProcessingChip label="Preview" status={getProcessingStatus(photo, 'preview')} step="preview" />
    ...
    const renderOverviewPage = () => (
        <>
            ...
            <div className="tools-panel tools-gallery-panel gallery-wrap card-glass reveal-up delay-1 gallery-studio">
                <div className="tools-panel-header">
                ...
                <div className="gallery-controls-surface">
                    <div className="gallery-toolbar">
                        <p className="gallery-meta-line" aria-live="polite">
                            <span className="gallery-meta-count">{overviewPhotos.length}</span>
                            <span className="gallery-meta-dim"> {plural(overviewPhotos.length, 'photo')} shown</span>
                            {selected.size > 0 && <span className="gallery-meta-dim"> · {selected.size} selected</span>}
                        </p>
                        <div className="gallery-tool-cluster">
                            {renderGallerySearchControl()}
    ...
    const renderBrowserWorkbenchPage = () => (
        <>
            <div className="tools-panel tools-workbench-panel">
                ...
                <div className="tools-control-group">
                    <span className="tools-control-label">Run steps</span>
                    {renderBrowserActionButtons()}
                </div>
            </div>
            ...
            <div className="tools-panel tools-gallery-panel gallery-wrap card-glass reveal-up delay-1 gallery-studio">
                ...
                <div className="gallery-controls-surface">
                    <div className="gallery-toolbar">
                        <p className="gallery-meta-line" aria-live="polite">
                            <span className="gallery-meta-count">{workbenchPhotos.length}</span>
                            <span className="gallery-meta-dim"> {plural(workbenchPhotos.length, 'photo')} in view</span>
                            {selectedVisibleCount > 0 && <span className="gallery-meta-dim"> · {selectedVisibleCount} selected</span>}
                        </p>
                        <div className="gallery-tool-cluster">
                            {renderGallerySearchControl()}
                        </div>
                ...
                <div ref={workbenchWindow.innerRef} className="gallery-grid" style={workbenchWindow.innerStyle}>
                    {workbenchWindow.visibleItems.map((photo, localIndex) => {
                        ...
                        if (photo.filename !== pendingFocusFilenames[0]) {
                            return tile;
                        }
                        return (
    ...
    const renderHistoryPage = () => (
        <div className="tools-panel tools-history-panel">
            <div className="people-panel-header">
                <div className="people-panel-title">History</div>
                <div className="people-panel-meta">Recent workbench runs</div>
            </div>
            {history.length === 0 ? (
                <EmptyState icon={<ArrowPathIcon />} title="No runs yet" message="Actions you run from the Workbench will show up here." />
            ) : (
                <div className="people-merge-history-list">
                    {history.map((h) => (
                        <div key={h.actionId} className="people-merge-history-row">
