...
import { WORKBENCH_URL_FILENAME_CAP, libraryFocusHref, workbenchFilenameHref, workbenchFilenamesHref } from './PhotoQuickActions';
                ...
                {screen === 'menu' ? (
                    <>
                        ...
                        <div className="action-sheet-list">
                            ...
                            {showWorkbenchLink && (
                                <button type="button" className="action-sheet-row" onClick={() => {
                                    if (!isBulk) {
                                        goTo(workbenchFilenameHref(filenames[0]));
                                        return;
                                    }
                                    const targets = filenames.slice(0, WORKBENCH_URL_FILENAME_CAP);
                                    if (filenames.length > WORKBENCH_URL_FILENAME_CAP) {
                                        showToast(`Opening the first ${WORKBENCH_URL_FILENAME_CAP} of ${filenames.length} selected photos in Workbench.`);
                                    }
                                    goTo(workbenchFilenamesHref(targets));
                                }}>
