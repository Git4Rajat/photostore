    ...
    for (let offset = 0; offset < file.size; offset += CLIENT_RAW_PREVIEW_SCAN_CHUNK_BYTES) {
        ...
        for (let index = 0; index < bytes.length; index += 1) {
            ...
            if (previousByte === 0xff) {
                ...
                // 0xFF00 is byte-stuffing (a literal 0xFF inside entropy-coded scan
                // data) and 0xFFD0-0xFFD7 are restart markers -- both are legal
                // inside a JPEG's compressed data and must not be mistaken for a
                // marker, or the scanner truncates the range well before the real
                // EOI (observed on real Apple ProRAW DNGs: the true embedded
                // preview's entropy data contains restart markers, so without this
                // guard the "match" ends ~300KB short of the actual image).
                if (value === 0x00 || (value >= 0xd0 && value <= 0xd7)) {
                    previousByte = -1;
                    continue;
                }
                if (value === 0xd8) {
                    // A fresh SOI while one is already "active" means the previously
                    // open span was bogus -- real JPEG streams never nest, so this
                    // restarts tracking from the new SOI instead of ignoring it.
                    // Without this, a stray 0xFFD8 byte pair occurring earlier in the
                    // file (e.g. inside a DNG's compressed RAW sensor tile data, which
                    // sits immediately before the real embedded PreviewImage on Apple
                    // ProRAW files) permanently poisons the scan: the real preview's
                    // own SOI is then silently ignored, and the "range" that closes on
                    // the next EOI-like byte pair is garbage spanning raw tile bytes
                    // plus part of the real image, which some browsers still decode
                    // (bitmap dimensions come from whatever SOF marker happens to fall
                    // inside it) but render as a corrupted/flat-grey image.
