...
// Reported while acquireBrowserAiModel() is downloading/warming a cold (uncached)
// model so the UI can show real progress instead of an indeterminate spinner.
export type BrowserAiLoadStage = {
    key: string;
    label: string;
    index: number;
    total: number;
