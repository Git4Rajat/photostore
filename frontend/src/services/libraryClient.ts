...
// Step 1 of "Clean library": owner re-enters their password (password mode
// only) and an emailed, single-use confirmation link is sent to the owner —
// plus one other member, picked at random, if the library is shared.
export const requestLibraryClean = async (password?: string): Promise<CleanRequestResult> => {
    const response = await authedFetch('/api/library/clean/request', {
        method: 'POST',
        body: JSON.stringify(password ? { password } : {}),
    });
    if (!response.ok) {
        throw new Error(await parseError(response, 'Could not start the cleanup confirmation.'));
    }
    return response.json();
};

export interface CleanConfirmResult {
    status: string;
    jobId?: string;
    queued?: boolean;
}

// Step 2: each required approver opens their emailed link (see
// ConfirmLibraryCleanPage) and this is called while signed in as that account.
export const confirmLibraryClean = async (token: string): Promise<CleanConfirmResult> => {
    const response = await authedFetch('/api/library/clean/confirm', {
        method: 'POST',
        body: JSON.stringify({ token }),
    });
    if (!response.ok) {
        throw new Error(await parseError(response, 'Could not confirm the cleanup.'));
    }
    return response.json();
};

export interface CleanStatusResult {
    status: string;
    result?: { photosDeleted?: number; blobsDeleted?: number; blobErrors?: number };
    error?: string;
}

export const getLibraryCleanStatus = async (jobId: string): Promise<CleanStatusResult> => {
    const response = await authedFetch(`/api/library/clean/status?jobId=${encodeURIComponent(jobId)}`);
    if (!response.ok) {
        throw new Error(await parseError(response, 'Could not check cleanup status.'));
    }
    return response.json();
};
...
export interface CleanupInfo {
    ...
    lastCleanupPhotosDeleted: number;
    lastCleanupBlobsDeleted: number;
    lastCleanupError: string;
}

export const getLibraryCleanupInfo = async (): Promise<CleanupInfo> => {
    const response = await authedFetch('/api/library/cleanup-info');
    if (!response.ok) {
        throw new Error(await parseError(response, 'Could not check cleanup info.'));
    }
    return response.json();
};
