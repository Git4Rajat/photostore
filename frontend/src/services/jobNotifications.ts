...
// 'skipped' (ipwork-only -- lease lost to another racer, or the photo was
// already fully processed by the time this message was dequeued) is just as
// final as 'done'/'failed': nothing further will happen to that job. Treating
// it as non-terminal left the "Processing photos…" pill lit for up to
// JOB_STATUS_WINDOW_MINUTES after the backend went fully idle, since 'both'
// mode's racing produces a steady stream of these on any active library
// (confirmed live 2026-09-14: every recent ipwork job was status='skipped',
// reason='already_done', with zero actual in-flight work).
export const TERMINAL_JOB_STATUSES = new Set(['done', 'failed', 'skipped']);
